...
class TextEncoder(nn.Module):
    """Token embedding + small Transformer encoder."""
    ...
    def __init__(
        ...
        *,
        ...
        self.pos_embedding = nn.Parameter(torch.zeros(1, max_length, text_dim))
    ...
    def forward(
        ...
        if text_tokens.ndim != 2:
            raise ValueError("text_tokens must have shape [batch, seq_len]")

        _, seq_len = text_tokens.shape
        if seq_len > self.pos_embedding.size(1):
            raise ValueError(
                f"Sequence length {seq_len} exceeds max_length {self.pos_embedding.size(1)}"
            )

        x = self.token_embedding(text_tokens) + self.pos_embedding[:, :seq_len]
        padding_mask = None if text_mask is None else ~text_mask.bool()
...
class TextConditioning(nn.Module):
    """Project text embeddings into scale/shift terms."""
...
class FiLMResidualBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.text_cond = TextConditioning(text_dim, out_channels)
    ...
    def forward(
        ...
        scale, shift = self.text_cond(text_emb)
        h = h + time_term
...
class TextConditionedUNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.mid_block1 = FiLMResidualBlock(
            bottleneck_channels,
            bottleneck_channels,
            time_dim,
            text_dim,
        ...
        self.mid_block2 = FiLMResidualBlock(
            bottleneck_channels,
            bottleneck_channels,
            time_dim,
            text_dim,
    ...
    def forward(
        ...
        for stage in self.down_stages:
            x, skip = stage(x, time_emb, text_emb)
            skips.append(skip)

        x = self.mid_block1(x, time_emb, text_emb)
        x = self.mid_attn(x)
        x = self.mid_block2(x, time_emb, text_emb)

        for stage, skip in zip(self.up_stages, reversed(skips)):
            x = stage(x, skip, time_emb, text_emb)
